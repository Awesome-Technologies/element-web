...
// ORIGINAL CODE
// https://github.com/element-hq/matrix-react-sdk/blob/v3.113.0/src/components/views/room_settings/RoomProfileSettings.tsx
// ORIGINAL PATH
// matrix-react-sdk/src/components/views/room_settings/
...
import { htmlSerializeFromMdIfNeeded } from "matrix-react-sdk/src/editor/serialize";
import { idNameForRoom } from "matrix-react-sdk/src/components/views/avatars/RoomAvatar";
...
export default class RoomProfileSettings extends React.Component<IProps, IState> {
    ...
    public constructor(props: IProps) {
        ...
        // check if there is a TI-M event for the topic
        let topicEvent = room.currentState.getStateEvents("de.gematik.tim.room.topic", "");
        if (!topicEvent) {
            topicEvent = room.currentState.getStateEvents(EventType.RoomTopic, "");
        }
        const topic = topicEvent && topicEvent.getContent() ? topicEvent.getContent()["topic"] : "";

        // check if there is a TI-M event for the name
        let nameEvent = room.currentState.getStateEvents("de.gematik.tim.room.name", "");
        if (!nameEvent) {
            nameEvent = room.currentState.getStateEvents(EventType.RoomName, "");
        }
    ...
    private saveProfile = async (e: React.FormEvent): Promise<void> => {
        ...
        if (this.state.originalDisplayName !== this.state.displayName) {
            // check if there is a TI-M event for the name
            if (room?.currentState.getStateEvents("de.gematik.tim.room.name", "")) {
                // change the TI-M room name
                await client.sendStateEvent(this.props.roomId, "de.gematik.tim.room.name", { name: displayName });
            } else {
                // change the standard room name
        ...
        if (this.state.originalTopic !== this.state.topic) {
            ...
            // check if there is a TI-M event for the topic
            if (room?.currentState.getStateEvents("de.gematik.tim.room.topic", "")) {
                // change the TI-M room name
                const content = ContentHelpers.makeTopicContent(this.state.topic, html);
                await client.sendStateEvent(this.props.roomId, "de.gematik.tim.room.topic", content);
            } else {
                // change the standard room topic
