...
// ORIGINAL CODE
// https://github.com/element-hq/matrix-react-sdk/blob/v3.113.0/src/components/views/dialogs/InviteDialog.tsx
// ORIGINAL PATH
...
import { Icon as EmailPillAvatarIcon } from "matrix-react-sdk/res/img/icon-email-pill-avatar.svg";
...
import { humanizeTime } from "matrix-react-sdk/src/utils/humanize";
import { IInviteResult, inviteMultipleToRoom, showAnyInviteErrors } from "matrix-react-sdk/src/RoomInvite";
...
import { selectText } from "matrix-react-sdk/src/utils/strings";
import Field from "matrix-react-sdk/src/components/views/elements/Field";
import TabbedView, { Tab, TabLocation } from "matrix-react-sdk/src/components/structures/TabbedView";
import Dialpad from "matrix-react-sdk/src/components/views/voip/DialPad";
import QuestionDialog from "matrix-react-sdk/src/components/views/dialogs/QuestionDialog";
import Spinner from "matrix-react-sdk/src/components/views/elements/Spinner";
import BaseDialog from "matrix-react-sdk/src/components/views/dialogs/BaseDialog";
import DialPadBackspaceButton from "matrix-react-sdk/src/components/views/elements/DialPadBackspaceButton";
...
import { ScreenName } from "matrix-react-sdk/src/PosthogTrackers";
...
import {
    ...
    ThreepidMember,
} from "matrix-react-sdk/src/utils/direct-messages";
        ...
        if (this.props.onRemove) {
            closeButton = (
                <AccessibleButton className="mx_InviteDialog_userTile_remove" onClick={this.onRemove}>
                    <img
                        src={require("matrix-react-sdk/res/img/icon-pill-remove.svg").default} // eslint-disable-line @typescript-eslint/no-var-requires
    ...
    private toggleMember = (member: Member): void => {
        if (!this.state.busy) {
            ...
            if (idx >= 0) {
                ...
                // requirement TI-Messenger client 5.2.1 - only allow one member to be invited in a createRoom-Event
                if (this.props.kind === InviteKind.Dm && targets.length > 0) {
                    targets = [];
                }
                targets.push(member);
    ...
    private renderSection(kind: "recents" | "suggestions" | "practitioner" | "organizations"): ReactNode {
        ...
        switch (kind) {
            ...
            case "practitioner":
                ...
                sectionName = _t("tim|invite|practitioner");
                break;
            case "organizations":
                ...
                sectionName = _t("tim|invite|organizations");
                break;
        }

        if (this.props.kind === InviteKind.Invite && kind === "recents") {
            sectionName = _t("invite|suggestions_section");
        }
