...
import Modal, { IHandle } from "matrix-react-sdk/src/Modal";
...
import { getAddressType } from "matrix-react-sdk/src/UserAddress";
import { VIRTUAL_ROOM_EVENT_TYPE } from "matrix-react-sdk/src/call-types";
import SpaceStore from "matrix-react-sdk/src/stores/spaces/SpaceStore";
import { makeSpaceParentEvent } from "matrix-react-sdk/src/utils/space";
import { JitsiCall, ElementCall } from "matrix-react-sdk/src/models/Call";
...
export default async function createRoom(client: MatrixClient, opts: IOpts): Promise<string | null> {
    ...
    if (opts.roomType) {
        ...
        // set the standard room type for TI-Messenger
        opts.roomType = "de.gematik.tim.roomtype.default.v1";
    }
    ...
    // create the default initial state for TI-M
    ...
    if (opts.createOpts?.name) {
        // add custom event to initial state
        createOpts.initial_state.push({
            type: "de.gematik.tim.room.name",
            //room_id: "",
            state_key: "",
            content: {
                name: opts.createOpts?.name,
            },
        });

        // remove name from standard createRoom event
        opts.createOpts.name = "";
    }

    // room topic event
    if (opts.createOpts?.topic) {
        // add custom event to initial state
        createOpts.initial_state.push({
            ...
            state_key: "",
            content: {
                topic: opts.createOpts?.topic,
            },
        });

        // remove topic from standard createRoom event
        opts.createOpts.topic = "";
    }
